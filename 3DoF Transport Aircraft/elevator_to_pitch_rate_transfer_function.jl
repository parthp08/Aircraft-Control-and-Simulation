## Transport Aircraft Elevator to Pitch Rate Response Transfer Function
...
C_matrix = reshape([0, 0, 0, 1, 0], 1,5)
D_matrix = [0]
...
sys_tf = tf(sys_ss)
# println(sys_tf)
## -0.011008118902439135*s^4 - 0.00719089113450555*s^3 - 0.00032401934173098904*s^2 - 2.858839452321571e-8*s + 5.971582278142817e-20
## ----------------------------------------------------------------------------------------------------------------------------------
...
# println(sys_tf_zpk)
##                        (1.0*s + 0.6045542739077264)(1.0*s + 0.048592556824546655)(1.0*s + 8.840393940333504e-5)(1.0*s - 2.088813393112813e-12)
## -0.011008118902439135  -------------------------------------------------------------------------------------------------------------------------------------------
...
# print_matrix(k_tf)
## Zeros = [
## Array{Complex{Float64},1}[[-0.604554+0.0im, -0.0485926+0.0im, -8.84039e-5+0.0im, 2.08881e-12+0.0im]]
## ]
## Poles = [
...
title!("Step response for openloop Elevator to Pitch Rate T.F")
xlabel!("Time(seconds)")
ylabel!("Pitch Rate")
ylims!(-0.1,0.1)
### Root Locus plot
p2 = rlocusplot(sys_tf)
title!("Root Locus plot for openloop Elevator to Pitch Rate T.F(full state)")
xlims!(-4,1)
ylims!(-2,2)
# ### Bode Plot
gr()    # pyplot backend doesnt work for bode plot
p3 = bodeplot(sys_tf)
title!("Bode plot for openloop Elevator to Pitch Rate T.F(full state)")
pyplot()

###========================================================================================
########### Short Period
z_sp = [2.08881e-12+0.0im]
p_sp = [-0.591239+0.881004im, -0.591239-0.881004im]
k_sp = -0.011008118902439135
...
p1 = plot(t, y, legend=false, reuse=false)
title!("Step response for openloop Elevator to Pitch Rate T.F(Short Period)")
xlabel!("Time(seconds)")
ylabel!("Pitch rate")
# ### Root Locus plot
p2 = rlocusplot(sys_tf_sp)
title!("Root Locus plot for openloop Elevator to Pitch Rate T.F(Short Period)")
xlims!(-4,1)
ylims!(-2,2)
# # ### Bode Plot
gr()    # pyplot backend doesnt work for bode plot
p3 = bodeplot(sys_tf_sp)
title!("Bode plot for openloop Elevator to Pitch Rate T.F(Short Period)")
# pyplot()
...
k_p = -0.011008118902439135
sys_tf_p = zpk(z_p, p_p, k_p)
gr()    # pyplot backend doesnt work for bode plot
p5 = bodeplot(sys_tf_p)
title!("Bode plot for openloop Elevator to Pitch Rate T.F(Phugoid)")
